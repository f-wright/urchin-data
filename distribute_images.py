...
def distribute_videos_and_images():
    ...
    # define outputs
    output_dirs = [os.path.join(CURRENT_DIR, OUTPUT_DIR, labeler, FILE_CATEGORY) for labeler in LABELERS]
    ...
    for path_i in range(len(images_list)):
        ...
        shutil.move(new_path, os.path.join(image_output_dirs[path_i % num_groups], filename))
    ...
    for path_i in range(len(videos_list)):
        ...
        shutil.move(new_path, os.path.join(video_output_dirs[path_i % num_groups], filename))
